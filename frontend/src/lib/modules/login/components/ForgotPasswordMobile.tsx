...
export const ForgotPasswordMobile: React.FC<ForgotPasswordMobileProps> = ({ isLoading, error, forgotPassword }) => {
    ...
    return (
        <AuthLayout
            title="Lupa Sandi"
            subtitle="Masukkan email untuk atur ulang kata sandi"
            icon={<Mail className="w-8 h-8" />}
        >
            <form onSubmit={formik.handleSubmit} className="flex flex-col gap-5 px-2">
                <div className="flex flex-col gap-2">
                    <label className="text-slate-700 text-sm font-semibold leading-normal">Alamat Email</label>
                    <div className="relative group">
                        <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 text-lg" />
                        <input
                            name="email"
                            type="email"
                            ...
                            } bg-slate-50 focus:bg-white focus:border-primary h-12 placeholder:text-slate-400 pl-11 pr-4 text-base font-normal leading-normal transition-all`}
                ...
                <button
                    ...
                    className="flex w-full cursor-pointer items-center justify-center rounded-xl h-12 px-5 btn-green text-white text-base font-bold shadow-md disabled:opacity-50"
                >
                    {isLoading ? (
